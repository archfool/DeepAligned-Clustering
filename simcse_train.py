...
def load_paras():
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, OurTrainingArguments), allow_abbrev=False)
...
def set_log(data_args, training_args, model_args, logging):
    # Setup logging
    logging.basicConfig(
...
def main():
    ...
    data_args, training_args, model_args = load_paras()

    # Set logging before initializing model.
    set_log(data_args, training_args, model_args, logging)
    ...
    if training_args.do_eval:
        ...
        if trainer.is_world_process_zero():
            with open(output_eval_file, "w") as writer:
                ...
                for eval_stepskey, value in sorted(results.items()):
                    logger.info(f"  {key} = {value}")
                    writer.write(f"{key} = {value}\n")
